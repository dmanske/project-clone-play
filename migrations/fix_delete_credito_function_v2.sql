-- Versão corrigida da função para deletar crédito
-- Esta versão remove APENAS passageiros que foram criados via crédito
...
BEGIN
  ...
  -- 1. PRIMEIRO: Limpar TODAS as referências ao crédito em viagem_passageiros
  -- Isso deve ser feito ANTES de tentar deletar o crédito para evitar erro de foreign key
  ...
  RAISE NOTICE 'TODAS as referências ao crédito removidas de viagem_passageiros';
...
COMMENT ON FUNCTION delete_credito_with_cleanup IS 'Deleta um crédito e remove apenas passageiros que foram criados via crédito';