-- Versão FINAL corrigida da função para deletar crédito
-- Esta versão resolve o problema de foreign key constraint
...
BEGIN
  ...
  -- 1. PRIMEIRO: Identificar e remover passageiros que foram CRIADOS via crédito
  -- Fazer isso ANTES de limpar as referências para manter a integridade
  ...
  LOOP
    ...
    -- Contar quantos foram removidos nesta iteração
    GET DIAGNOSTICS v_passageiros_removidos = ROW_COUNT;
    v_total_removidos := v_total_removidos + v_passageiros_removidos;
  ...
  RAISE NOTICE 'Total de passageiros removidos: %', v_total_removidos;

  -- 2. SEGUNDO: Limpar TODAS as referências restantes ao crédito
  -- Isso garante que não haverá violação de foreign key
  ...
  RAISE NOTICE 'Referências ao crédito limpas em viagem_passageiros';
  ...
  -- 5. QUINTO: Agora podemos deletar o crédito sem erro de foreign key
  DELETE FROM cliente_creditos 
  WHERE id = p_credito_id;

  RAISE NOTICE 'Crédito deletado com sucesso. Total de passageiros removidos: %', v_total_removidos;
...
COMMENT ON FUNCTION delete_credito_with_cleanup IS 'Deleta um crédito removendo passageiros criados via crédito e limpando todas as referências';